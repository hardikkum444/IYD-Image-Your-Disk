...
def encrypt(key, message, keytype):
    ...
    data = init_vector + cipher.encrypt(padded_message)
...
def decrypt(key, message, keytype):
    ...
    decrypted_data = decryptor.decrypt(message[AES.block_size:])
...
def encrypt_file(key, keytype, input_file, output_file):
    with open(input_file, 'rb') as file:
        message = file.read()
    encrypted_data = encrypt(key, message, keytype)
...
def encrypt_directory(key, keytype, directory_path, output_directory):
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith('.gz'):  # You can change the file extension as needed
